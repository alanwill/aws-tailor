...
def handler(event, context):
    ...
    claResponseArn = os.environ['TAILOR_SNSARN_CLARESPONSE']

    scanClaStatus = claStatus.scan(
        FilterExpression=Attr('state').eq('IN_PROGRESS')
    )

    for i in scanClaStatus['Items']:
        ...
        describeCreateAccountStatusPayload = {"CreateAccountRequestId": i['claRequestId']}

        endpoint, headers, data = sig_v4_post(aws_access_key_id=aws_access_key_id,
                                              aws_secret_access_key=aws_secret_access_key,
                                              aws_session_token=aws_session_token,
                                              payload=describeCreateAccountStatusPayload)
        describeCreateAccountStatus = requests.post(endpoint, headers=headers, data=data)
        responseData = json.loads(describeCreateAccountStatus.content)
        print(responseData)

        createAccountStatusState = responseData['CreateAccountStatus']['State']

        # During a failure there would be no accountId populated, catch for that.
        try:
            accountId = responseData['CreateAccountStatus']['AccountId']
        except KeyError as e:
            print(e)
            print("No accountId returned")

        if createAccountStatusState == 'SUCCEEDED':

            # Update talr-claStatus table with CLA state
            updateclaStatus = claStatus.update_item(
                Key={
                    'requestId': requestId
                },
                UpdateExpression='SET #state = :val1',
                ExpressionAttributeNames={'#state': "state"},
                ExpressionAttributeValues={':val1': createAccountStatusState}
            )

            # Push message to talr-director
            publishToTalrClARequest = sns.publish(
                TopicArn=claResponseArn,
                Message='{"linkedAccountId": "' + accountId +
                        '", "accessRoleName": "PayerAccountAccessRole", "linkedAccountEmail": "' +
                        accountEmailAddress + '"}'
            )

        elif createAccountStatusState != 'SUCCEEDED' or createAccountStatusState != 'IN_PROGRESS':

            createAccountStatusFailureReason = responseData['CreateAccountStatus']['FailureReason']
            # Update talr-claStatus table with CLA state
            updateclaStatus = claStatus.update_item(
                Key={
                    'requestId': requestId
                },
                UpdateExpression='SET #state = :val1, #failureReason = :val2',
                ExpressionAttributeNames={'#state': "state", '#failureReason': "failureReason"},
                ExpressionAttributeValues={':val1': createAccountStatusState,
                                           ':val2': createAccountStatusFailureReason}
            )
            raise Exception("Organizations createAccounts returned an error ", createAccountStatusFailureReason)
...
def sig_v4_post(payload, aws_access_key_id, aws_secret_access_key, aws_session_token):
    ...
    amz_target = 'AWSOrganizationsV20161128.DescribeCreateAccountStatus'
