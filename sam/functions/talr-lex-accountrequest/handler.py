...
def isvalid_payer(payer_alias):
    valid_divisions = ['abc']
    return payer_alias.lower() in valid_divisions
...
def validate_account_request(slots):
    accountCbAlias = try_ex(lambda: slots['accountCbAlias'])
    accountRegulated = try_ex(lambda: slots['accountRegulated'])
    accountVpcAzCount = try_ex(lambda: slots['accountVpcAzCount'])
    accountVpcPrefix = try_ex(lambda: slots['accountVpcPrefix'])
    accountRegion = try_ex(lambda: slots['accountRegion'])
    accountTagCostCenter = try_ex(lambda: slots['accountTagCostCenter'])
    accountTagLongProjectName = try_ex(lambda: slots['accountTagLongProjectName'])
    accountTagShortProjectName = try_ex(lambda: slots['accountTagShortProjectName'])
    accountTagEnvironment = try_ex(lambda: slots['accountTagEnvironment'])
    accountUserAccessList = try_ex(lambda: slots['accountUserAccessList'])
    accountTechnicalContactFullName = try_ex(lambda: slots['accountTechnicalContactFullName'])
    accountTechnicalContactUsername = try_ex(lambda: slots['accountTechnicalContactUsername'])
    requestorFullName = try_ex(lambda: slots['requestorFullName'])
    requestorUsername = try_ex(lambda: slots['requestorUsername'])
    requestorManager = try_ex(lambda: slots['requestorManager'])
    requestorDepartment = try_ex(lambda: slots['requestorDepartment'])
    requestorEmailAddress = try_ex(lambda: slots['requestorEmailAddress'])
    comment = try_ex(lambda: slots['comment'])

    if accountCbAlias and not isvalid_payer(accountCbAlias):
        return build_validation_result(
            False,
            'accountCbAlias',
            'We currently do not support {} as a valid division.  Can you try a different division?'.format(accountCbAlias)
        )
...
def account_request(intent_request):
    """
    Performs dialog management and fulfillment for creating a new account.
    ...
    accountCbAlias = try_ex(lambda: intent_request['currentIntent']['slots']['accountCbAlias'])
    accountRegulated = try_ex(lambda: intent_request['currentIntent']['slots']['accountRegulated'])
    accountVpcAzCount = try_ex(lambda: intent_request['currentIntent']['slots']['accountVpcAzCount'])
    accountVpcPrefix = try_ex(lambda: intent_request['currentIntent']['slots']['accountVpcPrefix'])
    accountRegion = try_ex(lambda: intent_request['currentIntent']['slots']['accountRegion'])
    accountTagCostCenter = try_ex(lambda: intent_request['currentIntent']['slots']['accountTagCostCenter'])
    accountTagLongProjectName = try_ex(lambda: intent_request['currentIntent']['slots']['accountTagLongProjectName'])
    accountTagShortProjectName = try_ex(lambda: intent_request['currentIntent']['slots']['accountTagShortProjectName'])
    accountTagEnvironment = try_ex(lambda: intent_request['currentIntent']['slots']['accountTagEnvironment'])
    accountUserAccessList = try_ex(lambda: intent_request['currentIntent']['slots']['accountUserAccessList'])
    accountTechnicalContactFullName = try_ex(lambda: intent_request['currentIntent']['slots']['accountTechnicalContactFullName'])
    accountTechnicalContactUsername = try_ex(lambda: intent_request['currentIntent']['slots']['accountTechnicalContactUsername'])
    requestorFullName = try_ex(lambda: intent_request['currentIntent']['slots']['requestorFullName'])
    requestorUsername = try_ex(lambda: intent_request['currentIntent']['slots']['requestorUsername'])
    requestorManager = try_ex(lambda: intent_request['currentIntent']['slots']['requestorManager'])
    requestorDepartment = try_ex(lambda: intent_request['currentIntent']['slots']['requestorDepartment'])
    requestorEmailAddress = try_ex(lambda: intent_request['currentIntent']['slots']['requestorEmailAddress'])
    comment = try_ex(lambda: intent_request['currentIntent']['slots']['comment'])

    # nights = safe_int(try_ex(lambda: intent_request['currentIntent']['slots']['Nights']))

    session_attributes = intent_request['sessionAttributes']

    # Load confirmation history and track the current account request.
    accountRequest = json.dumps({
    ...
    session_attributes['currentAccountRequest'] = accountRequest

    if intent_request['invocationSource'] == 'DialogCodeHook':
        # Validate any slots which have been specified.  If any are invalid, re-elicit for their value
        validation_result = validate_account_request(intent_request['currentIntent']['slots'])
    ...
    log.debug('bookHotel under={}'.format(reservation))
...
def book_car(intent_request):
    ...
    log.debug('bookCar at={}'.format(reservation))
...
def dispatch(intent_request):
    ...
    log.debug('dispatch userId={}, intentName={}'.format(intent_request['userId'],
                                                         intent_request['currentIntent']['name']))

    intent_name = intent_request['currentIntent']['name']

    # Dispatch to your bot's intent handlers
    if intent_name == 'AccountRequest':
        return account_request(intent_request)
...
def handler(event, context):
    ...
    log.debug('event.bot.name={}'.format(event['bot']['name']))
